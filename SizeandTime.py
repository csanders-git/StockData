...
API_KEY = "demokey"
DATA_FILE = 'public_breach_size.csv'
...
def process_file(csv_headers, csv_reader):
	...
	onek = csv_headers.index("<100K")
	onem = csv_headers.index("<1,000,000")
	hundm = csv_headers.index("<100000000")
	fhundm = csv_headers.index("<500000000")
	gtfhundm = csv_headers.index(">500000000")
	...
	for row in csv_reader:
		if row[symbol_index] == '' or row[publication_index] == '' or row[still_find] == 'TRUE':
			continue
		breach_size = [0,0,0,0,0]
		if row[onek] == "TRUE":
			breach_size[0] = breach_size[0] + 1
		if row[onem] == "TRUE":
			breach_size[1] = breach_size[1] + 1
		if row[hundm] == "TRUE":
			breach_size[2] = breach_size[2] + 1
		if row[fhundm] == "TRUE":
			breach_size[3] = breach_size[3] + 1
		if row[gtfhundm] == "TRUE":
			breach_size[4] = breach_size[4] + 1
		...
		return_data.append({"symbol": row[symbol_index], "day": pub_day, "month" : pub_month, "year" : pub_year, 'combined_date' : row[publication_index], 'breach_size': breach_size })
	return return_data
...
def main():
	...
	breach_size = ['onek','onem','hundm','fhundm','gtfhundm']
	for breach_size_index in range(0,len(breach_size)):
		OUT_FILE = "breach-data-" + breach_size[breach_size_index] +  ".csv"
		...
		for breach in symbol_data:
			if breach['breach_size'][breach_size_index] == 1:
				combined_date = datetime.datetime.strptime(breach['combined_date'], "%m/%d/%y")
				breach_date = combined_date.strftime("%Y-%m-%d")
				nasdaq_df = read_nasdaq('IXIC.csv', breach_date)
				print(breach_date)
				process_daydata(breach, nasdaq_df, breach_date, stats_csv_writer)
