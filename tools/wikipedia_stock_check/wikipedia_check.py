...
def parse_wikipedia_entry(wikipedia_result):
    """
    Takes the wikipedia entry and determines if there is stock info
    or if the organization is a subsidiary
    """
    ...
    if "type" in wikipedia_result and "traded_as" not in wikipedia_result:
        if wikipedia_result['type'].find("Subsidiary") > -1:
            ...
            # We should never get a parent page that isn't valid
...
def update_stock(headers, row, replace_text):
    """
    Update our datastructure version of the dataset
    """
    was_public_index = headers.index("Was_Public")
...
def main():
    """
    A quick script to enrich data with stock names from wikipedia
    """
